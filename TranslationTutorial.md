...
首先，将`BlockRacing.jar`放入plugins文件夹中，启动一遍服务器，plugins文件夹内会多出一个`BlockRacing`文件夹，其中存储着所有方块竞速的游戏配置。其中，lang文件夹下储存插件内语言文件，也就是上面提到的第二部分。

如果其中的语言并不是你需要的：

如果你需要其他语言，可以先下载中文版本的文件，然后将其内容复制给ChatGPT，让他翻译即可，他会自动识别颜色代码和占位符。
...
2. 在`.minecraft\assets\objects`文件夹中，搜索这个hash值，你就拿到了你的语言的翻译文件。将其复制到`BlockRacing/minecraftlang`文件夹中，后缀改为`.json`，名字改为你的语言代码。
